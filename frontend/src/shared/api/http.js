...
export async function downloadFile(path, { token, fileName } = {}) {
  ...
  const resolvedName = fileName || parseFileName(response.headers.get("content-disposition")) || "download";
...
export async function getFileObjectUrl(path, { token } = {}) {
  ...
  return {
    ...
    fileName: parseFileName(response.headers.get("content-disposition")) || "preview"
  };
}
